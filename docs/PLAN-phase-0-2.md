...
> **Completed 2026-08-17.** All 13 tasks executed; 49 tests green across
...
- [x] **Step 1: Write `packages/core/src/types.ts`**
...
- [x] **Step 5: Build and run against the real machine**
...
- [x] **Step 2: Commit and verify CI passes**
...
Running the built CLI against a real machine — 181 apparent extensions — exposed
three defects the fixture corpus could not have caught. All three were fixed
before this plan was marked complete.

**1. The cost model was wrong.** The plan charged a skill's entire body to every
turn, which produced a headline figure of 374,867 tokens — 187% of a 200K window,
an obviously impossible number. Agents keep only a skill's name and description
resident; the body loads when the skill is invoked. Cost is now split into
`tokens` (charged every turn) and `deferredTokens` (charged on invocation), and
both are reported. Corrected figures on the same machine: 5,758 per turn, 192,446
deferred.

**2. Discovery double-counted.** The same skill ships in both a marketplace clone
(`plugins/marketplaces/vercel/skills/workflow`) and a versioned cache
(`plugins/cache/vercel/vercel-plugin/0.45.1/skills/workflow`), and skills vendor
nested `upstream/SKILL.md` copies inside themselves. `findSkillFiles` now stops
descending once a directory contains a `SKILL.md`, and results are deduped by id.
181 reported extensions collapsed to 83 actual ones.

**3. `--json` truncated through a pipe.** `main().then(code => process.exit(code))`
tore down the process before an async stdout pipe flushed, cutting a 2 MB payload
at 66 KB. Writing to a file worked, which is why it was invisible until the output
was piped. Now sets `process.exitCode`.

**4. Windows path assertions.** The CI matrix caught `paths.test.ts` hard-coding
POSIX separators. The implementation was already correct — it uses `join` — so
only the test changed. This is precisely the regression the plan's Windows matrix
was included to catch.

Two further deviations were made deliberately during execution: a root
`tsconfig.json` with project references was added so `tsc --build` orders the
packages, and a Vitest alias resolves `@cupel/core` to source so the suite runs
